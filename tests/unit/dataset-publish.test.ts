...
describe("JSON shard round-trip (db → publisher → JSON.parse)", () => {
  ...
  it("nests word_glosses + translations under each verse in the shard", () => {
    const siva = JSON.parse(
      readFileSync(join(buildDir, "json", "siva-sutras.json"), "utf8"),
    ) as {
      chapters: Array<{
        verses: Array<{
          verse: number;
