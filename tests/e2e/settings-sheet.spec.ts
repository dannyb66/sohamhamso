...
test.describe('settings sheet (Aa)', () => {
  ...
  test('changing the theme to Dark sets <html data-theme="dark">', async ({ page }) => {
    await page.goto(VERSE_URL);
    await page.waitForLoadState('networkidle');
    const trigger = page.locator('[data-settings-trigger]').first();
  ...
  test('changing font size updates the --text-base CSS variable on <html>', async ({ page }) => {
    await page.goto(VERSE_URL);
    await page.waitForLoadState('networkidle');
    const trigger = page.locator('[data-settings-trigger]').first();
    if (!(await trigger.count())) {
  ...
  test('localStorage `sohamhamso:settings` persists and survives reload', async ({ page }) => {
    await page.goto(VERSE_URL);
    await page.waitForLoadState('networkidle');
    const trigger = page.locator('[data-settings-trigger]').first();
    if (!(await trigger.count())) {
    ...
    await sheet.locator('input[type="range"][aria-label="Font size in pixels"]').fill('20');
