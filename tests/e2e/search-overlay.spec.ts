...
test.describe("search overlay (⌘K combobox)", () => {
  ...
  test("⌘K (or Ctrl+K) opens the overlay from the homepage", async ({
    page,
    browserName,
  }) => {
    await page.goto("/");
  ...
  test("clicking [data-search-trigger] in the masthead opens the overlay", async ({
    page,
  }) => {
    await page.goto("/");
    await page.waitForLoadState("networkidle");
  ...
  test("typing a query surfaces at least one result row", async ({
    page,
    browserName,
  }) => {
    await page.goto("/");
  ...
  test("Enter on the first result navigates to that verse page", async ({
    page,
    browserName,
  }) => {
    await page.goto("/");
    ...
    if (!probe) {
      test.info().annotations.push({
        type: "skip",
        description:
          "search backend returned no usable data (see docs/test-findings.md)",
      });
      return;
    }
