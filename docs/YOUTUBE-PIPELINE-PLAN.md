...
# Chapter Videos (Format 2) — shipped 2026-06-10

A second distribution format on the same pipeline: one **16:9 (1920×1080 @
30fps) video per (text, chapter)** that plays every verse sequentially —
Devanāgarī + IAST + translation on screen, Google TTS narration of the
translation per verse, narrated title card, end-screen-safe outro with the
canonical chapter-page URL. Chapter rows share the `videos` table
(`format='chapter'`, `verse_num=0`), the R2 bucket (`chapters/` prefix), and
Cron B; rendering runs on its own cron (A2, `youtube-generate-chapters.yml`,
03:00/15:00 UTC, `--limit=2`, 330-min timeout). Operator runbook:
`scripts/youtube/README.md` → Chapter videos; system map:
`pipeline/youtube/ARCHITECTURE.md`.

## Revised governance (premise gate, 2026-06-10)

1. **English-only v1: 8 chapter videos** — siva-sutras ch1–3, spanda-karikas
   ch1–4, pratyabhijna-hrdayam ch1. All language plumbing stays config-driven
   (`chapters.langs` in `data/youtube-config.yaml`), so expansion is config +
   backfill dispatch, not code.
2. **The 12-language fan-out is deferred** until ALL THREE clear:
   (a) the chapter format proves **retention** on the English 8,
   (b) the **multi-audio-track question** is answered (below), and
   (c) **per-language reviewer coverage** exists.
3. **Multi-audio-track test (1 video):** on the FIRST uploaded chapter video,
   add a `hi` audio track via YouTube Studio (track generated by
   `youtube-render-chapters.ts --audio-only --video-id=N --lang=hi`,
   sample-exact against the English segment timing). Corrected expectations:
   the feature is **Studio-only and Advanced-features-gated** (no API path,
   availability not guaranteed at this channel size); it localizes **audio +
   metadata, not on-screen pixels** — so it answers per-language *audio*
   demand only, not the full "8 uploads vs 96" question. If the Studio option
   is unavailable, record that here and weigh per-language uploads against
   the kill-switch data instead.
4. **Uploads after the measurement window.** Chapter uploads must not begin
   until the running 30-day shorts measurement window closes (they would
   contaminate the shorts kill-switch metrics). Enforced in code, not policy:
   Cron B holds `approved` chapter rows behind `chapters.uploads_enabled:
   false`; renders/QA/R2 staging proceed regardless. The operator flips the
   flag after confirming the window-close date (window-close checklist in the
   operator handbook).
5. **No corpus mutation.** Chapters use a per-format translation floor
   (`chapters.min_translation_status: draft`) — shorts keep `reviewed`,
   `STATUS-CONTRACT.md` semantics are untouched, and the backfill requires a
   non-empty translation per verse with explicit status-priority selection
   (published → reviewed → draft).
6. **Analytics first (M0.5, shipped):** the real
   `scripts/youtube-analytics-sync.ts` + the 3-scope OAuth union
   (`youtube.upload` + `yt-analytics.readonly` + `youtube.force-ssl`) landed
   before chapter uploads; Cron E is re-enabled with full state-DB plumbing.
   Metric honesty is contractual: the API fills views / avg-view-duration /
   avg-view-percentage / subscribers-gained; **CTR and 3s-retention are
   Studio-only and UTM clicks are site-side — those columns stay NULL**, so
   the kill-switch readout never claims unmeasured metrics. Site-side UTM
   ingestion remains a named gap.

### Multi-audio-track test outcome (M3 item 15 — record here)

> _Pending. After running the Studio walkthrough on the first uploaded
> chapter video, record: date, whether the Studio audio-track surface was
> available, listener verdict on the hi track fit, and the resulting
> 12-lang fan-out decision (multi-audio tracks vs per-language uploads vs
> freeze)._

## Schema / determinism deltas

- `videos.format TEXT NOT NULL DEFAULT 'short' CHECK(format IN
  ('short','chapter'))` + `idx_videos_format_status`; chapter rows use
  `verse_num = 0` (corpus verses start at 1 — the existing UNIQUE key holds).
  Guarded ALTER migration in `scripts/youtube-db-ensure.ts`; the live state DB
  is stamped `PRAGMA user_version = 2` and `getVideosDb` **refuses a pre-M0
  checkout against a post-M0 DB** (no stale-branch `verse_num=0` leakage).
- `video_analytics.audio_lang` added for the multi-audio test (NULL until it
  runs).
- **Chapter determinism:** `translation_md5` holds
  `determinism.ts::chapterContentMd5()` — md5 of the per-verse **manifest**
  `{verse_num, devanagari, iast, translation_text, translation_row_id,
  tts_voice_id}` ordered by verse_num, so a verse edit, translation-row swap,
  voice change, or reordering all cascade a chapter re-render.
  `template_version` holds `CHAPTER_TEMPLATE_VERSION = 'c1'`
  (`pipeline/youtube/versions.ts`) — an independent version track from the
  shorts `TEMPLATE_VERSION = 'v2'`; bumps never cross-supersede.
- **One-time cascade exemption:** landscape support in shared
  `Background`/`Footer`/`Translation` used default-preserving props — Short
  output stays byte-identical, shorts `TEMPLATE_VERSION` NOT bumped, locked by
  the golden-frame test + `template-version.test.ts`.
- `translation_row_id` on a chapter row anchors to the FIRST verse's
  translation row (documented anchor; FK satisfied); full per-verse provenance
  lives in the R2 `.meta.json` sidecar, written as an **atomic pair** with the
  mp4 (incl. `outroStartS` for the final timestamp line).
- Queue hardening: the shorts cron filters `format='short'` on both pending
  and failed picks; chapter render queue excludes `last_error_phase='upload'`
  rows; md5-mismatch at render → supersede + skip (never the 3×90-min retry
  loop); lease TTL 4h for chapter rows; `youtube-render-chapters.ts` exits
  non-zero on MAX_RETRY so failure issues fire; `youtube-status.ts` gains
  `byFormat` counts.

## Format ceiling — VBT does NOT fit this engine

Vijnana Bhairava Tantra ch1 = **163 verses ≈ 55–70 minutes ≈ beyond the 6h
GHA job ceiling** at observed render rates. This engine does not scale to VBT
without chunked `frameRange` renders or a bigger runner. **Do not backfill VBT
into this pipeline when its licensing clears — it needs its own plan.** (The
163-verse figure is also the synthetic worst case the timing math is
unit-tested against.)

## Ops checklist (chapter rollout)

1. **Channel verification** — verify @sohamhamso at youtube.com/verify before
   flipping uploads (siva-sutras ch3 exceeds 15 min; unverified channels fail
   with a named no-retry error).
2. **Re-auth on the 3-scope union** — `youtube.upload` +
   `yt-analytics.readonly` + `youtube.force-ssl` in one consent
   (`youtube-oauth-setup.ts` prints granted scopes and warns on short
   grants). The old "upload-only" scope instruction is obsolete and would
   regress Cron E, revisit flip-public, and the supersede sweep.
3. **M1 pacing gate** — sample renders (spanda ch2 + siva-sutras ch1 excerpt)
...
4. **Staged rollout** — `backfill --dry-run` locally (read-only) →
   ...
   (R2 state push is last-writer-wins).
5. **Window close** — confirm the shorts window date → flip
   `chapters.uploads_enabled: true` → commit/push → Cron B uploads unlisted →
   Studio review (timestamps render as YouTube chapters, HD source) → end
   screens/cards per video → `youtube-revisit.ts --visibility=public`.
6. **Multi-audio test** on the first upload; record the outcome above.
7. **"Watch this chapter on YouTube" site CTA** — still open: needs the
   video-ID manifest (youtube-state.db → static manifest at site build); the
   chapter pages carry a TODO slot for it.

## Contributor quickstart (zero secrets)

```bash
bun install

# Day-0: real chapter MP4s, no credentials (TTHW < 10 min)
MOCK_ALL=true bun run youtube:render-chapter-samples
# → samples-chapters/spanda-karikas-ch2.mp4 (+ .timing.json)
# → samples-chapters/siva-sutras-ch1.mp4    (first 8 verses — dead-air stress test)

# Unit suite (chapter timing, determinism, QA, queue, metadata, migration):
bun run youtube:test
```

- **Fixtures:** `tests/unit/youtube/_db-helpers.ts` is the shared fixture
  module — `buildTempDb()` (in-memory state DB at current schema),
  `seedChapterCorpus()` + `SPANDA_CH1_VERSES` / `SPANDA_CH4_VERSES`
  (multi-verse and 1-verse chapters), `seedChapterVideo()`,
  `makeChapterSidecar()`, and `syntheticNarrationDurationsS(n)` (45/163-verse
  timing inputs). New chapter tests should build on these rather than
  hand-rolling rows.
- **Staying off live systems:** `MOCK_ALL=true` never constructs TTS/R2/
  YouTube clients (canned WAV, local temp dir instead of R2, fake video ids,
  canned analytics report, canned chapter sidecar). `--dry-run` plans without
  side effects. The only dangerous local operation is running
  `youtube-backfill-chapters.ts` WITHOUT `--dry-run` against a pulled state
  DB — don't; state mutation goes through `workflow_dispatch` (the R2 state
  push is last-writer-wins).
